...
// The site URL isn't in the JWE cookie any more (we trimmed it in Phase 3
// to keep the cookie under the 4KB browser limit), so we re-fetch it from
// accessible-resources when we need to build a clickable Confluence URL.
// Cache-friendly per Skill 4 run — callers pass the returned pair back to
// the tool handlers rather than re-hitting the endpoint per hit.
...
function buildPageWebUrl(siteUrl, webui) {
  ...
  // Confluence API returns webui like "/spaces/SD/pages/1234/Snowflake+landing"
  // — needs the "/wiki" prefix and the site URL.
